...
// (c) 2007 Tjark Weber
//  Modified 2021 R. E. Bryant.  Generate constraints in both directions
// ****************************************************************************
...
int main(int argc, char** argv) {
    ...
    printf("c \n");
    printf("c Modified to add constraints in the other direction:\n");
    printf("c We also have n clauses which say that a hole has to contain some pigeon\n");
    printf("c Then for each pigeon we have a set of clauses\n");
    printf("c ensuring that only a single hole can contain it.\n");
    printf("c \n");
    printf("c This encoding leads to a total of (n+1) * n propositional variables and\n");
    printf("c (n+1) + n * (n * (n+1) / 2)   +   n + (n+1) * ((n-1) * n) / 2\n");
    printf("c \n");
    printf("c The resulting SAT problem is unsatisfiable.\n");
    printf("c \n");
    int vars = (n+1) * n;
    int clauses = (n+1) + n    * (n * (n+1))/2;
    clauses +=     n +   (n+1) * ((n-1) * n)/2;
    printf("p cnf %d %d\n", vars, clauses);
    ...
    // n clauses which say that a hole has to contain some pigeon
    for (j=1; j <= n; j++) {
	for (i=1; i <= n+1; i++) {
	    int index;
	    index  = n*(i-1)+j;
    ...
    // for each pigeon we have a set of clauses ensuring that only one single
    // hole contains it
    for (i=1; i <= n+1; i++) {
	for (j=1; j <= n-1; j++) {
	    for (k=j+1; k <= n; k++) {
