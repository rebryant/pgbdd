...
class ExpansionVariable:
    ...
    def __init__(self, id, rootVariable, literalList):
        self.id = id
    ...
    def split(self, id, uvar):
        ...
        vcomp = ExpansionVariable(id, self.rootVariable, self.literalList + [-uvar])
        return (vtrue, vcomp)
...
class ExpandedClause:
    ...
    def comment(self, outfile):
        slist = [str(l) for l in self.source]
        outfile.write("# Expansions of [%s]" % (", ".join(slist)))

    def expand(self, uvar, trueMap, compMap):
        nlist = []
